...
\begin{tikzpicture}[node distance=5mm and 5mm]
  \node[block,text width=17mm] (ctrl) {Frozen $K$\\4 / 16 / 35};
  \node[block,text width=19mm,right=of ctrl] (buf) {Ranked buffer\\mode, rank, bits};
  \node[block,text width=18mm,right=of buf] (sched) {$P$-way scheduler\\$bP+l<K$};
  \draw[flow] (ctrl)--(buf); \draw[flow] (buf)--(sched);
  \node[hw,text width=16mm,below=10mm of ctrl] (pred) {Prediction\\+ residual};
  \node[hw,text width=16mm,right=of pred] (fwd) {Forward DST\\pass 1/2};
  \node[hw,text width=16mm,right=of fwd] (quant) {Quantize};
  \node[hw,text width=16mm,below=of quant] (deq) {Dequantize};
  \node[hw,text width=16mm,left=of deq] (inv) {Inverse DST\\pass 1/2};
  \node[hw,text width=16mm,left=of inv] (sse) {Reconstruct\\+ SSE};
  \node[hw,text width=16mm,below=of sse] (rd) {$D+\lambda R$\\RD cost};
  \node[result,text width=21mm,right=10mm of rd] (tree) {Balanced min tree\\cost, stable rank};
  \foreach \a/\b in {pred/fwd,fwd/quant,quant/deq,deq/inv,inv/sse,sse/rd,rd/tree}
    \draw[flow] (\a)--(\b);
  \draw[flow] (sched.south) -- ++(0,-4mm) -| (pred.north);
\end{tikzpicture}
\caption{Implemented RTL with up to 16 serial 132-cycle lanes. Exact HM rate
is external; hardware CABAC is out of scope.}
\label{fig:rtl}
\end{figure}
...
balanced tree replaces the P-deep scan. The 31-bit cost is safe for the observed
maximum 1,312,532,898 only; it is not an arbitrary-input proof.
...
\begin{table*}[!b]
\caption{Routed Microarchitecture Results}
\label{tab:routed}
\centering
\footnotesize
\setlength{\tabcolsep}{3.5pt}
...
\textbf{Shared serial, P16 tree} & \textbf{16} & \textbf{Routed} &
\textbf{61,207} & \textbf{816} & \textbf{$-$9.680 ns} &
\textbf{68.120 MHz} & \textbf{8.134 M/s} \\
\bottomrule
\end{tabularx}
\par\vspace{2pt}
\footnotesize Vivado 2024.2, \texttt{xck26-sfvc784-2LV-c}, 5 ns
out-of-context constraint. $^\dagger$Synthesized LUT count; placement did not
run. No failed value is interpolated.
\end{table*}
...
AI assistance was used in preparing this artifact.
